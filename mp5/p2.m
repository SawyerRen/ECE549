...
%% 加载2张立体图像
left = imread('data/kitchen_left.jpg');
right = imread('data/kitchen_right.jpg');
sizeI = size(left);
%% 基本的块匹配
...
%                        blockDiffs(blockIndex, 1) = blockDiffs(blockIndex, 1) + blockDiff;
                    % SSD
%                         blockDiff = sqrt(abs(rightI(j, k) - leftI(j, k + i)));
%                         blockDiffs(blockIndex, 1) = blockDiffs(blockIndex, 1) + blockDiff;
...
%中值滤波，窗口选择25*25较为合适
%  DbasicSubpixel_2 = medfilt2(DbasicSubpixel,[15 15]);
